...
const HASHLINE_PREFIX_PLUS_RE = /^\+\s*(?:\d+\s*#\s*|#\s*)[ZPMQVRWSNKTXJBYH]{2}:/;
...
function normalizeFuzzyLine(text) {
    return text
        .trimEnd()
...
function isFuzzyEquivalentLine(expected, actual) {
    return normalizeFuzzyLine(expected) === normalizeFuzzyLine(actual);
}
// ─── Parsing ────────────────────────────────────────────────────────────
function diagnoseLineRef(ref) {
...
function parseAnchorRef(ref) {
    ...
    if (!HASH_ALPHABET_RE.test(hash)) {
        throw new Error(`[E_BAD_REF] Invalid line reference "${ref}": hash uses invalid characters, hashes use alphabet ${NIBBLE_STR} only.`);
    }
...
// ─── Mismatch formatting ────────────────────────────────────────────────
function formatMismatchError(mismatches, fileLines, retryLines = new Set()) {
    const retryLineSet = new Set(retryLines);
 ...
 */
function assertNoDisplayPrefixes(lines) {
    for (const line of lines) {
        ...
        if (HASHLINE_PREFIX_RE.test(line) ||
            ...
            throw new Error(`[E_INVALID_PATCH] "lines" must contain literal file content, not rendered "LINE#HASH:" or diff "+/-" prefixes. Offending line: ${JSON.stringify(line)}`);
        }
    }
}
/**
 ...
 */
function hashlineParseText(edit) {
    const lines = edit ?? [];
...
function assertEditItem(edit, index) {
    const unknownKeys = Object.keys(edit).filter((key) => !ITEM_KEYS.has(key));
    if (unknownKeys.length > 0) {
        throw new Error(`Edit ${index} contains unknown or unsupported fields: ${unknownKeys.join(", ")}.`);
    }
    ...
    if (edit.op !== "replace" &&
        ...
        throw new Error(`[E_BAD_OP] Edit ${index} uses unknown op "${edit.op}". Expected "replace", "append", "prepend", or "replace_text".`);
    }
    ...
    if (edit.op === "replace_text") {
        if (typeof edit.oldText !== "string" || typeof edit.newText !== "string") {
            throw new Error(`[E_BAD_OP] Edit ${index} with op "replace_text" requires string "oldText" and "newText" fields.`);
        }
    ...
    if (edit.op === "replace" && typeof edit.pos !== "string") {
        throw new Error(`[E_BAD_OP] Edit ${index} with op "replace" requires a "pos" anchor string.`);
    }
    if ((edit.op === "append" || edit.op === "prepend") && "end" in edit) {
        throw new Error(`[E_BAD_OP] Edit ${index} with op "${edit.op}" does not support "end". Use "pos" or omit it for file boundary insertion.`);
    }
}
export function resolveEditAnchors(edits) {
    const result = [];
    for (const [index, edit] of edits.entries()) {
        assertEditItem(edit, index);
        const op = edit.op;
        switch (op) {
            case "replace": {
                result.push({
...
function maybeWarnSuspiciousUnicodeEscapePlaceholder(edits, warnings) {
 ...
 */
function warnBareHashPrefixLines(edits, fileLines, warnings) {
    ...
    for (const edit of edits) {
        ...
        for (const line of edit.lines) {
            ...
            if (match)
                suspects.push({ line, hash: match[1] });
        }
    }
    ...
    if (matchCount > 0 || suspects.length >= 2) {
        const matchHint = matchCount > 0
            ...
            : "";
        warnings.push(`${suspects.length} edit line(s) start with a 2-char hash and ":" (e.g. ${JSON.stringify(suspects[0].line)}).${matchHint} If you copied these from "read" output, they are hash prefixes, not file content — resend "lines" as literal content.`);
    }
}
...
function assertDoesNotEmptyFile(originalContent, result) {
    if (originalContent.length > 0 && result.length === 0) {
        throw new Error("[E_WOULD_EMPTY] Refusing to empty a non-empty file through edit. If intentional, use the write tool or bash.");
    }
}
...
function throwEditConflict(left, right, reason) {
    throw new Error(`[E_EDIT_CONFLICT] Conflicting edits in a single request: edit ${left.index} (${left.label}) and edit ${right.index} (${right.label}) ${reason}. Merge them into one non-overlapping change or split the request.`);
}
function computeInsertionBoundary(edit, lineIndex) {
    switch (edit.op) {
        case "append": {
...
function findExactUniqueTextMatch(content, oldText) {
    if (oldText.length === 0) {
    ...
    for (let index = 1; index < matches.length; index++) {
        if (matches[index] - matches[index - 1] < oldText.length) {
            throw new Error("[E_MULTI_MATCH] replace_text found overlapping exact matches; re-read and use hashline edits.");
        }
    }
    if (matches.length === 0) {
    ...
    if (matches.length > 1) {
        throw new Error("[E_MULTI_MATCH] replace_text found multiple exact matches in the current file. Re-read and use hashline edits.");
    }
...
function resolveEditToSpan(edit, index, content, lineIndex, noopEdits) {
    ...
    switch (edit.op) {
        case "replace": {
            ...
            if (originalLines.length === edit.lines.length &&
        ...
        case "prepend": {
            ...
            return {
                ...
                boundary: computeInsertionBoundary(edit, lineIndex),
                ...(content.length === 0
                    ? { insertMode: "prepend-empty-origin" }
                    : {}),
            };
        }
        case "replace_text": {
...
function assertNoConflictingSpans(spans) {
    for (let leftIndex = 0; leftIndex < spans.length; leftIndex++) {
        ...
        for (let rightIndex = leftIndex + 1; rightIndex < spans.length; rightIndex++) {
            ...
            if (insertSpan.start >= replaceSpan.start &&
 ...
 */
function validateAnchorEdits(edits, lineIndex, warnings, signal) {
    const mismatches = [];
    const retryLines = new Set();
    const acceptedFuzzyRefs = new Set();
        ...
        if (ref.textHint !== undefined) {
            ...
            if (hintedHash === ref.hash &&
                ...
                if (!acceptedFuzzyRefs.has(key)) {
                    ...
                    warnings.push(`Accepted fuzzy anchor validation at line ${ref.line}: exact hash mismatched, but the copied line content still matched after whitespace/Unicode normalization.`);
                }
                return true;
            }
        }
            ...
            case "replace": {
                if (edit.end) {
                    if (edit.pos.line > edit.end.line) {
                        throw new Error(`[E_BAD_OP] Range start line ${edit.pos.line} must be <= end line ${edit.end.line}`);
                    }
                ...
                if (!edit.end && edit.lines.length > 1) {
                    warnings.push(`Single-anchor replace at ${describeEdit(edit)} swapped only line ${edit.pos.line}, but you supplied ${edit.lines.length} replacement lines. If you meant to replace a range, add end. If you meant to expand one line into many, ignore this.`);
                }
                ...
                if (nextLine !== undefined &&
                    ...
                    warnings.push(`Potential boundary duplication after ${describeEdit(edit)}: the replacement ends with a line that matches the next surviving line after trim.`);
                }
                ...
                if (prevLine !== undefined &&
                    ...
                    warnings.push(`Potential boundary duplication before ${describeEdit(edit)}: the replacement starts with a line that matches the preceding surviving line after trim.`);
                }
                break;
            }
            case "append": {
                if (edit.pos && !validate(edit.pos))
                    continue;
                if (edit.lines.length === 0) {
                    throw new Error("[E_BAD_OP] Append with empty lines payload. Provide content to insert or remove the edit.");
                }
                break;
            }
            case "prepend": {
                if (edit.pos && !validate(edit.pos))
                    continue;
                if (edit.lines.length === 0) {
                    throw new Error("[E_BAD_OP] Prepend with empty lines payload. Provide content to insert or remove the edit.");
                }
                break;
            }
            case "replace_text":
 ...
 */
function resolveEditSpans(edits, content, lineIndex, noopEdits, signal) {
    ...
    return [...resolvedSpans].sort((left, right) => {
        ...
        if (left.kind === "insert" && right.kind === "insert") {
            return ((right.boundary ?? -1) - (left.boundary ?? -1) ||
 ...
 */
function assembleEditResult(content, spans, signal) {
    let result = content;
 ...
 */
export function applyHashlineEdits(content, edits, signal) {
    throwIfAborted(signal);
    if (!edits.length)
    ...
    const { mismatches, retryLines } = validateAnchorEdits(workingEdits, lineIndex, warnings, signal);
    ...
    const orderedSpans = resolveEditSpans(workingEdits, content, lineIndex, noopEdits, signal);
...
export function computeAffectedLineRange(params) {
    const { firstChangedLine, lastChangedLine, resultLineCount, contextLines = ANCHOR_CONTEXT_LINES, maxOutputLines = ANCHOR_MAX_OUTPUT_LINES, } = params;
...
export function formatHashlineRegion(lines, startLine) {
    const lineNumberWidth = String(startLine + Math.max(0, lines.length - 1)).length;
    return lines
        .map((line, index) => {
 ...
 */
export function computeChangedLineRange(original, result) {
    if (original === result)
        return null;
    ...
    if (firstDiff === minLen && original.length === result.length)
        return null;
    ...
    function indexToLine(charIdx, text) {
        let line = 1;
