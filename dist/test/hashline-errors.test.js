...
import { applyHashlineEdits, computeLineHash, detectLineEnding, normalizeToLF, resolveEditAnchors, restoreLineEndings, stripBom, } from '../src/index.js';
...
test('replace supports single lines and ranges and rejects invalid ranges', () => {
    ...
    assert.throws(() => apply('a\nb', [{ op: 'replace', pos: ref(2, 'b'), end: ref(1, 'a'), lines: ['x'] }]), /E_BAD_OP/);
...
test('validates operation shapes and anchor hash syntax', () => {
    ...
    assert.throws(() => resolveEditAnchors([{ op: 'append', end: ref(1, 'a'), lines: ['x'] }]), /E_BAD_OP/);
});
