...
function secondAnchor(preview: string): string {
...
test('rejects unsafe classifications without writing', async () => {
  ...
  for (const bytes of [Buffer.from([0x61, 0, 0x62]), Buffer.from([0x61, 0xc3, 0x28])]) {
    ...
    await assert.rejects(
      () => client.edit({ path, edits: [{ op: 'prepend', lines: ['unsafe'] }] }),
      /E_(?:BINARY_FILE|DECODE_LOSS)/,
    );
...
test('preserves CRLF, UTF-8 BOM, and an existing permission mode', async (t) => {
  ...
  if (canAssertMode) await chmod(path, 0o640);
...
test('replacement failure leaves the original intact and removes the temporary file', async () => {
  ...
  await assert.rejects(
    () => client.edit({ path, edits: [{ op: 'replace', pos: secondAnchor(preview), lines: ['patched'] }] }),
...
test('filesystem client supports every edit operation and returns classified failures', async () => {
  ...
  assert.match(
    await client.edit({ path, edits: [{ op: 'replace_text', oldText: 'missing', newText: 'x' }] }),
    /^\[E_NO_MATCH\]/,
  );
  assert.match(
    await client.edit({ path, edits: [{ op: 'replace', pos: '1#ZZ:stale', lines: ['x'] }] }),
    /^\[E_STALE_ANCHOR\]/,
  );
});
